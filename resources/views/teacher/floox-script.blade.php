...
<div id="modal_messageModal" class="modal">
    ...
    <div class="modal-footer">
        ...
        <a href="{{url('teacher/messages')}}" class="waves-effect waves-green btn-flat green white-text left">go to messages</a>
    </div>
</div>
...
<script type="text/javascript">
    <?php
$appointments_today = Teachat\Models\Appointments::with(['parent'])->where(['teacher_id' => \Auth::user()->id, 'appt_date' => Carbon\Carbon::now()->toDateString()])->orderBy('appt_date', 'ASC')->get()->toArray();
...
?>
    ...
    floox.connect(function(){
        ...
        @foreach($appointments_today as $app)
            arrayOfUserId.push("{{ $app['parent_id'] }}");
        @endforeach

        @foreach($parents as $p)
            arrayOfUserId.push("{{ $p->id }}");
        @endforeach
        ...
        floox.onCallStateChange(function(state, data){
            ...
            switch(state) {
                ...
                case Floox.States.CallStates.ACCEPTED:
                    ...
                    $('.calling').closeModal();
                    $('#incoming_call').closeModal();
                    window.location.href="{{url('/')}}/teacher/videocall/"+data.userType+"/"+$('#user_id').val()+"/"+data.session;
        ...
        $('.callParent').click(function(){
            floox.call($(this).attr('data-parent-id'));
        ...
        floox.messageReceived(function(data){
            if(window.location.pathname == '/teacher/messages'){
                ...
                message += '<li class="collection-item avatar"><a href="/teacher/messages"><span class="title truncate">'+data.from.additional_data.first_name+' '+data.from.additional_data.last_name+'</span><p class="truncate">'+data.message+'</p><small>'+createdAt+'</small></a></li>';
