        ...
        <div class="msgs">
            <div class="row" style="position: relative;">
                ...
                <div class="col s12 m4 l4 white-text contact_names">
                    ...
                    <div class="contact_content blue-grey darken-1">
                        <ul>
                            ...
                            <li class="c_content waves-effect msg_select_user blue-grey darken-4" id="10406382" onclick="timerCheckerRedirectVideoChat(10406382 );">
                            ...
                            @foreach($parents as $p)
                                <?php $profile_img = ($p->profile_img != null) ? "https://s3-ap-southeast-1.amazonaws.com/teachatco/images/".$p->profile_img : "".asset('/images/profile/dp.png').""; ?>
                                <li class="c_content waves-effect msg_select_user" id="{{ $p->id }}" onclick="getUserMessages({{ $p->id }}, 0, null, true);">
                                    <figure class="online userProfileimg" style="background-image: url('{{ $profile_img }}')"><span class="unread hide">0</span></figure>
                                    <span class="contact_name truncate"> {{ $p->first_name.' '.$p->last_name }} </span>
                                    <span class="contact_status" id="{{ $p->id }}">
                                    <i class="fa fa-fw fa-circle offline"></i> <span class="textStatus">Offline</span>
                                    </span>
                                </li>
                            @endforeach
                ...
                <div class="col s12 m8 l8 contact_msgs">
                    ...
                    <div class="msg_body grey lighten-3 body_msg-content disabled" id="message_msgPanel">
    ...
    <script type="text/javascript">
        ...
        $('#messages_submit').on('submit',function(e){
            ...
            if($('#file_chat').get(0).files.length != 0){
                $.ajax({
                    url: "{{url('teacher/uploadAttachment')}}", // Url to which the request is send
        ...
        function sendMessage(val, attachment) {
            ...
            floox.getLoggedInUsers([opp], function(users){
                if(users.length == 0){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ url('teacher/messages/sendEmailIfOffline') }}",
                        data: data,
                        cache: false,
