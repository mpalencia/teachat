    ...
    @extends('theme')
    @section('title_tag')
        <title>Teacher : Edit Appointment</title>
                ...
                <div class="col s12  m12 l9">
                    <div class="profile-content">
                        <div class="row">
                            ...
                            <div class="row">
                                <form id="edit-appointments_submit" accept-charset="utf-8">
                                    <div class="input-field col s12 m12">
                                        <select class="parent_select validate" required="" name="parent_id" id="parent_id">
                                            <option value="0" selected>-- Choose a parent -- </option>
                                            @foreach($parents as $parent)
                                                <option value="{{ $parent['id'] }}">{{ $parent['name_prefix'] }}{{ $parent['first_name'] }} {{ $parent['last_name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="input-field col s12 m4">
                                        <input id="appointment_date" type="text" readonly="" required="" class="" name="appt_date" value="{{ $appt[0]['appt_date'] }}">
                                        <label for="appointment_date">Appointment Date</label>
                                    </div>
                                    <div id="appoinment_time">
                                        <div class="input-field col s12 m4">
                                            <input placeholder="Appointment Start Time" id="appointment_stime" type="text" name="appt_stime" value="{{ $appt[0]['appt_stime'] }}" class="time start" required="">
                                            <label for="appointment_stime">Appointment Start Time</label>
                                        </div>
                                        <div class="input-field col s12 m4">
                                            <input placeholder="Appointment End Time" id="appointment_etime" type="text" name="appt_etime" value="{{ $appt[0]['appt_etime'] }}" class="time end" required="">
                                            <label for="appointment_etime">Appointment End Time</label>
                                        </div>
                                    </div>
                                    <div class="input-field col s12 m12">
                                        <input id="appointment_title" type="text" class="validate" required="" name="title" value="{{ $appt[0]['title'] }}">
                                    ...
                                    <dir><input type="hidden" name="id" value="{{ $appt[0]['id'] }} "></input></dir>
                                    <!--<div class="input-field col s12">
    ...
    @section('additional_bodytag')
        ...
        <script type="application/javascript">
            ...
            $('#parent_id').val("{{ $appt[0]['parent_id'] }}");
            $('#appointment_description').val("{{ $appt[0]['description'] }}");
