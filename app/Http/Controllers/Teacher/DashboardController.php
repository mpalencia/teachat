...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        $data = array(
            ...
            'appointments_today' => $this->appointments->getAllByAttributesWithRelations(['teacher_id' => Auth::user()->id, 'appt_date' => Carbon::now()->toDateString()], ['teacher'], 'appt_date'),
            'announcements' => $this->announcements->getDashboardAnnoucements(Auth::user()->role_id, Auth::user()->school_id),
            'appt' => $this->appointments->getAllByAttributesWithRelations(['teacher_id' => Auth::user()->id, 'status' => 0], ['user'], 'created_at', 'DESC', 'not_null'),
        );
