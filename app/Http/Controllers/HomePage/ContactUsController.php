...
class ContactUsController extends Controller
{
    ...
    {
        $contactUs->merge(['first_name' => $contactUs->full_name, 'last_name' => '', 'email' => 'gab@incubixtech.com' /*'info@teachat.co'*/]);

        if ($mailSender->send('email.contact_us', 'Contact Us from Teachat.co', $contactUs->all())) {
            return response()->json(['success' => true, 'message' => 'Success! Your message has been sent to Teachat team.']);
        }
